...
Prereqs (manual, see plan): `docker compose up -d` (both DBMS, no auth), Ollama
...
def load_aikg_to_neo4j(json_path: Path, session_id: str) -> int:
    ...
    driver = GraphDatabase.driver(AIKG_NEO4J_URL, auth=None)  # container runs NEO4J_AUTH=none
    try:
